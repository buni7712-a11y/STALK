...
function selectStock(name, change) {
  stockInput.value = stockInput.value.replace('$', `@${name} `);
  stockDropdown.classList.add('hidden');
  stockBadge.classList.remove('hidden');
  stockBadgeText.innerText = `📌 연동된 종목: ${name} (${change})`;
}
...
function toggleReaction(type) {
  ...
  if(btn.classList.contains('text-emerald-400')) {
    // 활성화 상태면 해제
    btn.classList.remove('text-emerald-400', 'bg-emerald-500/20', 'border', 'border-emerald-500/40');
    btn.classList.add('text-gray-400', 'bg-zinc-800');
    countEl.innerText = count - 1;
  } else {
    // 비활성화 상태면 활성화
    btn.classList.add('text-emerald-400', 'bg-emerald-500/20', 'border', 'border-emerald-500/40');
    btn.classList.remove('text-gray-400', 'bg-zinc-800');
    countEl.innerText = count + 1;
  }
}
...
function toggleTheme() {
  ...
  // 화면 전체의 메인 배경색을 가지고 있는 최상단 div를 제어해야 합니다.
  // 여기서는 간단히 body의 배경색 톤을 교체하는 예시입니다.
  if(!isDark) {
    document.body.style.filter = "invert(1) hue-rotate(180deg)"; // 간단히 화면을 반전시켜 라이트모드 느낌을 내는 꿀팁
    themeText.innerText = "다크 모드로 보기";
  } else {
    document.body.style.filter = "none";
    themeText.innerText = "라이트 모드로 보기";
  }
}