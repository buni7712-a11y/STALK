// ==========================================================================
// STALK Core Engine - Production Release Version
// ==========================================================================
...
// CodePen/글로벌 환경에서 Supabase 객체를 안전하게 가져오는 프로토콜
const supabaseClient = window.supabase ? window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY) : null;

if(!supabaseClient) {
    console.error("Supabase 라이브러리가 아직 로드되지 않았습니다. HTML 상단을 확인하세요.");
}
...
// 어플리케이션 초기화 구동부
function initApp() {
  if (document.getElementById('user-nickname')) document.getElementById('user-nickname').innerText = userNickname;
  if (document.getElementById('user-points')) document.getElementById('user-points').innerText = userPoints;
  if (document.getElementById('user-avatar')) document.getElementById('user-avatar').innerText = userAvatar;
  ...
  // 주식 자동완성 이벤트 리스너 바인딩 안전 초기화
  initStockAutocomplete();
}

// 소셜 팔로우 개수 뷰포트 드라이버
function updateSocialCounts() {
  if (document.getElementById('following-count')) document.getElementById('following-count').innerText = followingList.length;
  if (document.getElementById('followers-count')) document.getElementById('followers-count').innerText = followersCount;
}

// 1. 프로필 관리 매니저
function updateProfile() {
  const inputEl = document.getElementById('changeNicknameInput');
  if (!inputEl) return;
...
function addNotification(message) {
  ...
  p.className = 'border-b border-zinc-800/60 pb-2 text-zinc-300 font-medium tracking-tight animate-fade-in';
  p.innerHTML = `✨ ${message}`;
...
function showSection(sectionId) {
  ...
  const activeMenu = document.getElementById(activeMenuMap[sectionId]);
  if (activeMenu) {
    activeMenu.className = "w-full text-left px-4 py-2.5 rounded-xl text-sm bg-zinc-800 text-white font-bold transition";
  }
}
...
function verifyStudent() {
  ...
  if (statusEl) statusEl.innerHTML = `인증 상태: <span class="text-neon font-bold">★ 학생 정주주 Verified</span>`;
  gainPoints(50);
...
// 다이렉트 메시지(DM) 윈도우 인터페이스 브릿지
function openDM(targetName) {
  activeChatPartner = targetName;
  showSection('dm-section');
  
  const headerEl = document.getElementById('dm-window-header');
  const inputEl = document.getElementById('dmInput');
  const btnEl = document.getElementById('dmSendBtn');

  if (headerEl) headerEl.innerText = `💬 [${targetName}] 주주님과의 DM 채널`;
...
function simulateVirtualMarket() {
  const el1 = document.getElementById('stock-price-1');
  const el2 = document.getElementById('stock-price-2');
  if(!el1 || !el2) return; // DOM 예외 안전 가드 장치
  ...
  el1.innerText = `${p1.toLocaleString()}원 (${change1 >= 0 ? '+' : ''}${change1}%)`;
  el1.className = `text-xs font-mono mt-0.5 ${change1 >= 0 ? 'text-red-400' : 'text-blue-400'}`;
  
  el2.innerText = `${p2.toLocaleString()}원 (${change2 >= 0 ? '+' : ''}${change2}%)`;
  el2.className = `text-xs font-mono mt-0.5 ${change2 >= 0 ? 'text-red-400' : 'text-blue-400'}`;
}
...
function updateVirtualUI() {
  const profitEl = document.getElementById('my-virtual-profit');
  const moneyEl = document.getElementById('my-virtual-money');
  if(!profitEl || !moneyEl) return;

  profitEl.innerText = `${myVirtualProfit >= 0 ? '+' : ''}${myVirtualProfit.toFixed(2)}%`;
  profitEl.className = `font-bold ${myVirtualProfit >= 0 ? 'text-red-400' : 'text-blue-400'}`;
  moneyEl.innerText = `${myVirtualMoney.toLocaleString()}원`;
}
...
async function loadPosts() {
  ...
  filteredData.forEach(post => {
    const likesCount = parseInt(localStorage.getItem(`likes_${post.id}`) || 0);
...
// Supabase 트랜잭션 안전 파이프라인 인서트 프로토콜
async function addPost() {
  const titleInput = document.getElementById('postTitle');
  const contentInput = document.getElementById('postContent');
  const catSelect = document.getElementById('postCategory');
  const opt1Input = document.getElementById('pollOption1');
  const opt2Input = document.getElementById('pollOption2');
...
// 4. 오답노트 보관 모듈
function addDiary() {
  const profitEl = document.getElementById('diaryProfit');
  const titleEl = document.getElementById('diaryTitle');
  const contentEl = document.getElementById('diaryContent');

  if (!profitEl || !titleEl || !contentEl) return;

  const profit = profitEl.value.trim();
...
function selectStock(name, change) {
  const stockInput = document.getElementById('stock-input');
  const stockDropdown = document.getElementById('stock-dropdown');
  const stockBadge = document.getElementById('selected-stock-badge');
  const stockBadgeText = document.getElementById('stock-badge-text');

  if(stockInput) stockInput.value = stockInput.value.replace('$', `@${name} `);
  if(stockDropdown) stockDropdown.classList.add('hidden');
  if(stockBadge) stockBadge.classList.remove('hidden');
  if(stockBadgeText) stockBadgeText.innerText = `📌 연동된 종목: ${name} (${change})`;
}
...
// 20번 기능: 주말 차트 게임 스크립트
function playChartGame(choice) {
  if(choice === '매도') {
    alert("정답입니다! 리먼 브라더스 파산으로 주가가 -45% 폭락했습니다. 인버스로 위기를 기회로 만드셨네요! 💸");
  } else {
    alert("아쉽습니다! 리먼 브라더스 사태로 자산이 -45% 토막 났습니다. 주말 대회를 통해 헤징을 배워보세요! 📉");
  }
}

// 21번 기능: 테마 토글 스크립트 (다크/라이트)
let isDark = true;
function toggleTheme() {
  ...
  if(!isDark) {
    ...
    if(themeText) themeText.innerText = "다크 모드로 보기";
  } else {
    document.body.style.filter = "none";
    if(themeText) themeText.innerText = "라이트 모드로 보기";
  }
}
...
// ==========================================
// 🛠️ 실시간 업로드 버그 수정 및 예외 방어 패치 (맨 밑에 추가)
// ==========================================

// 1. 피드 로드 함수 안전하게 덮어쓰기 (Overriding)
loadPosts = async function() {
  const feed = document.getElementById('feed');
  if (!feed) {
    console.warn("피드 뷰포트요소(#feed)가 현재 화면에 없어 렌더링을 안전하게 스킵합니다.");
    return; 
  }
...
// 2. 투자일기 로드 함수 안전하게 덮어쓰기 (Overriding)
loadDiaries = function() {
  const list = document.getElementById('diary-list');
  if (!list) {
    console.warn("일기장 박스(#diary-list)가 현재 화면에 없어 렌더링을 안전하게 스킵합니다.");
    return; 
  }
...
// 3. 타이머 변동 리스너 안전 가드 추가
const originalSimulateVirtualMarket = simulateVirtualMarket;
simulateVirtualMarket = function() {
  const el1 = document.getElementById('stock-price-1');
  const el2 = document.getElementById('stock-price-2');
  if (!el1 || !el2) return; // 요소가 화면에 없으면 연산 엔진 정지
  originalSimulateVirtualMarket();
}

// 4. 수정한 안전 로직 브라우저 엔진에 즉시 재반영
console.log("✅ STALK 업로드 버그 버스트 오버라이딩 패치 완료!");
loadPosts();
loadDiaries();